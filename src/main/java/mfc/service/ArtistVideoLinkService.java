...
public class ArtistVideoLinkService extends AbstractService<ArtistLink> {
    ...
    public int automaticAddVideoLinkFromYoutube(final String artistCode){
        ...
        if(artist == null){
           ...
           List<GoogleSearchResult> listFromGoogle = httpClient.responseFromGoogle(sb.toString());
           ...
           // 登録可能な動画リストを取得する
           List<ArtistLink> listForBatchInsert = getRagisterableVideoLinkList(listFromGoogle,movieListFromDb,artist.getArtistName1(),artistCode);

           if(listForBatchInsert == null || listForBatchInsert.isEmpty()){
               return 20;
           }
           // 登録可能な動画リストをバッチ挿入する
          int[] status =  jdbcManager.insertBatch(listForBatchInsert).execute();
    ...
    private List<ArtistLink> getRagisterableVideoLinkList(
            ...
            final List<ArtistLink> listFromDb,
            final String artistName,
            final String artistCode){
        List<ArtistLink> registerableMovieList = InstanceManager.newNotAtomicList();
        ...
        for(ArtistLink record : listFromDb){
            String videoKey = YouTubeManager.getMovieKeyOfYoutube(record.getLinkUrl());
        ...
        for(GoogleSearchResult node:listFromGoogle){
            ...
            if(isPvOrMvCheckOK(node.getTitle())){
                 if(LinkAllowRegisterManager.isYouTube(node.getUrl())){
                       String videoKeyFromGoogle = YouTubeManager.getMoviekeyOfYoutubeFromSearchResult(node.getUrl());
                       if(!StringUtil.isBlank(videoKeyFromGoogle)){
                           if(mapUrlForCheck.get(videoKeyFromGoogle) == null){
                               //System.out.println(YouTubeManager.trimForVideoTitle(node.getTitle(),artistName)+ "(Auto)");
                               //System.out.println(YouTubeManager.getNormalUrl() + videoKeyFromGoogle);
                               ArtistLink newRecord = new ArtistLink();
                               newRecord.setArtistCode(artistCode);
                               newRecord.setDispName(YouTubeManager.trimForVideoTitle(node.getTitle(),artistName)+ "(Auto)");
                               newRecord.setLinkUrl(YouTubeManager.getNormalUrl() + videoKeyFromGoogle);
                               newRecord.setUpdateUserId("Auto Registerd!!");
                               registerableMovieList.add(newRecord);
                               mapUrlForCheck.put(videoKeyFromGoogle, videoKeyFromGoogle);
                           }
                       }
                   }
            }
    ...
    private boolean isPvOrMvCheckOK(final String targetTitle){
        ...
        return WrapperRegexManager.isMatched(StringPrescribedManager.convert(targetTitle).toUpperCase(), "^.*(PV|MV|MUSICVIDEO|ミュージックビデオ).*$");
    ...
    public List<ArtistLink> fetcheList(final String artistCode){
        ...
        return select()
                .where(
                    new SimpleWhere()
                    .eq("artistCode", artistCode)
                ).getResultList();

    }

    public ArtistLink fetcheSingle(final String artistCode,final String  displayTitle){
        ...
        return select()
                .where(
                    new SimpleWhere()
                    .eq("artistCode", artistCode)
                    .eq("dispName", displayTitle)
                ).getSingleResult();

    }

    public int updateWithJudgement(final String artistCode,final String displayTitle,final String movieUrl){

        if(StringUtil.isBlank(artistCode) || StringUtil.isBlank(displayTitle)|| StringUtil.isBlank(movieUrl)){
            return 0;
        }
