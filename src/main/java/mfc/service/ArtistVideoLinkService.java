        ...
        if (checkStatus) {
            checkStatus = WrapperRegexManager
                    .isMatched(StringPrescribedManager.convert(targetTitle)
                            .toUpperCase(),
                            "^.*(PV|MV|MUSICVIDEO|MUSIC VIDEO|OFFICIAL VIDEO|OFFICIALVIDEO|ミュージックビデオ|ミュージック.ビデオ).*$");
            if (checkStatus) {
                boolean checkStatusOmit = WrapperRegexManager
                        .isMatched(
                                StringPrescribedManager.convert(targetTitle)
                                        .toUpperCase(),
                                "^.*(踊ッテミタ|新PV|新.PV|PV公開|PV.公開|PV解禁|PV.解禁|PV.秘密|PV作成|PV.作成|♫|THAITANIUM|♥|MMV CONCERT|"
                                        + "新シイMV|新タナMV|MVガ公開|MVヲ公開|MV解禁|MVヲ解禁|MVノ秘密|MVガ作成|MVヲ作成|"
                                        + "PV密着|PV.密着|PV画像|[『【「（〈\\(《｛\\[]AMV|SUDDENATTACK|手書.PV|手書PV|[『【「｛\\(（〈《\\[]OPV|"
                                        + "新MV|新.MV|MV公開|MV.公開|MV解禁|MV.解禁|MV.秘密|MV作成|マジシャンズレッド|"
                                        + "MV.作成|MV密着|MV.密着|SPECIAL MV|取材|カバー|コピー|主題歌|撮影|REACTION|"
                                        + "PV特集|MV特集|PV集|MV集|厳選PV|厳選MV|？|！|ANIME|MMD-PV|イメージPV|"
                                        + "ニュース|歌ッテミタ|吹奏楽|PV.MV.フル|MV.PV.フル|フル.MV|フル.PV|フル.FULL|小山ユウ|"
                                        + "吹奏楽|コメント|MV.MAKING|メイキング|MV.作.|[0-9]{1,2}月[0-9]{1,2}日|自作MV|"
                                        ...
                                        + "弾イテミタ|COVER[\\)\\]）』】」》｝]|COVER.MV|COVER.PV|COPY.MV|AMV|"
                                        + "COPY.PV|[『【「（｛〈\\(《\\[]MMD|MMD[\\)\\]）』】」》｝]|HD[\\)\\]）』】」》｝]|"
